  ...
  const changeLanguage = (til) => {
    ...
    useEffect(()=> {
        async function getData(){
            await axios.get(`${process.env.REACT_APP_BASE_TV_URL}/airing_today?api_key=${process.env.REACT_APP_API_KEY}&language=${t("Tili")}&page=1`)
            .then(ress => {
                    ...
                    return(
                       <div className="col-3 my-3 col-sm-6 col-md-3 col-xs-6">
                        <div className="card">
                            ...
                            <h5 style={{width: '100%'}} className='text-truncate p-2 mt-4'>{item.name}</h5>
                            <p style={{width: '100%'}} className='text-truncate p-2 mt-4'>{t("sana")}{` ${item.first_air_date}`}</p>
